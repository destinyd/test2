...
TuanApi.create [
  ...
  :name => "hao1231",
  :docfind => "//urlset/url",
:suite=>"d=d.children[5].children[1];d.each{|a|n[a.name]=a.content};p[:price]=n['price'];p[:type_id]=21;p[:finish_at]=Time.at(n['endTime'].to_i);p[:address]=n['city'];p[:title]=n['title'];tmp=p[:title].p[:title].gsub!(/<!\[CDATA\[(.*)\]\]>/,'\1');o=p[:outlinks_attributes]=[{:url=>n['loc']}];p[:uploads_attributes]=[{:image_file_name=>n['image']}];",
  :tuan_urls=> [
  ...
  :name => "hao1232",
  :docfind => "//urlset/url",
:suite=>"d=d.children[3].children[1];d.each{|a|n[a.name]=a.content};p[:price]=n['price'];p[:type_id]=21;p[:finish_at]=Time.at(n['endTime'].to_i);p[:address]=n['city'];p[:title]=n['title'];tmp=p[:title].p[:title].gsub!(/<!\[CDATA\[(.*)\]\]>/,'\1');o=p[:outlinks_attributes]=[{:url=>n['loc']}];p[:uploads_attributes]=[{:image_file_name=>n['image']}];",
  :tuan_urls=> [
